...
mean(abs(training_scores - X_train$TRX_COUNT)) ##14

# root mean squared error for training data
sqrt(mean((training_scores - X_train$TRX_COUNT)^2)) ##23

test_scores <- predict(tree_model, X_test)
write.table(test_scores, file = "test_predictions.csv", row.names = FALSE, col.names = TRUE)
prediction <- read.csv("test_predictions.csv", header = TRUE)
